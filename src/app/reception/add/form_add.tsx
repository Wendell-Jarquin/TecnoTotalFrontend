...
export default function AddReparacionForm() {
  ...
  const [equipoId, setEquipoId] = useState(""); // Solo para mostrar en el input
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
        <div>
          ...
          <select
            ...
            onChange={e => {
              ...
              setEquipoId(""); // Limpia el ID del equipo al cambiar de cliente
        ...
        <div>
          ...
          <select
            ...
            defaultValue=""
            onChange={e => {
              const selected = equipos.find(eq => String(eq.id) === e.target.value);
              setEquipoId(selected?.id || "");
              register("equipo").onChange(e);
            }}
          >
            <option value="" disabled>Seleccione un equipo</option>
            {equipos
              .filter(eq => eq.clienteId === clienteSeleccionado?.clientes_id)
              .map((equipo) => (
